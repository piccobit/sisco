...
func (tq *TagQuery) QueryService() *ServiceQuery {
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withService: tq.withService.Clone(),
...
func (tq *TagQuery) WithService(opts ...func(*ServiceQuery)) *TagQuery {
	query := &ServiceQuery{config: tq.config}
	for _, opt := range opts {
		opt(query)
	}
	tq.withService = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withService; query != nil {
		if err := tq.loadService(ctx, query, nodes,
			func(n *Tag) { n.Edges.Service = []*Service{} },
			func(n *Tag, e *Service) { n.Edges.Service = append(n.Edges.Service, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadService(ctx context.Context, query *ServiceQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Service)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ServiceTable)
		s.Join(joinT).On(s.C(service.FieldID), joinT.C(tag.ServicePrimaryKey[0]))
		s.Where(sql.InValues(joinT.C(tag.ServicePrimaryKey[1]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ServicePrimaryKey[1]))
