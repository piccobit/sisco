...
func (sq *ServiceQuery) QueryArea() *AreaQuery {
...
func (sq *ServiceQuery) WithArea(opts ...func(*AreaQuery)) *ServiceQuery {
	query := &AreaQuery{config: sq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (sq *ServiceQuery) loadArea(ctx context.Context, query *AreaQuery, nodes []*Service, init func(*Service), assign func(*Service, *Area)) error {
	...
	for i := range nodes {
		if nodes[i].area_services == nil {
			continue
		}
		fk := *nodes[i].area_services
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "area_services" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
